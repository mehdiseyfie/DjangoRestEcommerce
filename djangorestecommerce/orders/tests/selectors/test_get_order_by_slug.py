...
from djangorestecommerce.orders.selectors import get_order_by_slug
...
class GetOrderBySlugTest(TestCase):

    def test_returns_order_for_owner(self):
        profile = ProfileFactory()
        order = OrderFactory(customer=profile)

        result = get_order_by_slug(customer=profile, slug=order.slug)
    ...
    def test_raises_404_for_other_customer_order(self):
        ...
        with self.assertRaises(Http404):
            get_order_by_slug(customer=other, slug=order.slug)
